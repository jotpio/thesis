...
def iteratively_evaluate_dates_files(
    start_date,
    end_date,
    eval_date_function,
    load_only_challenge_data,
    challenges=True,
    only_successful=True,
    fast_runs_percentiles=None,
    verbose=False,
):
    """
    Iteratively evaluate date files in range start_date to end_date. On each date the eval_date_fuction is executed.
    Returns list of evaluation function returns (one for each date)
    """
    all_returns = []
    ...
    for single_date in daterange(dt_start_date, dt_end_date):
        ...
        # load day
        dates_dict = load_dates_from_npz(
            current_date_str,
            current_date_str,
            only_challenges=load_only_challenge_data,
            verbose=False,
        )
        # extend day data
        ...
        # filter runs
        if fast_runs_percentiles is not None:
            for id_fast_runs_percentile, fast_runs_percentile in enumerate(
                fast_runs_percentiles
            ):
                # apply evaluation function
                all_returns[id_fast_runs_percentile].append(
                    eval_date_function(
                        date_dict, challenges, only_successful, fast_runs_percentile
                    )
                )
        else:
            # apply evaluation function
            ...
            # apply evaluation function
            all_returns.append(
                eval_date_function(date_dict, challenges, only_successful, None)
            )

    return all_returns


def iteratively_collect_and_filter_dates_files(
    start_date,
    end_date,
    load_only_challenge_data,
    percentile_value=200,
    challenges=True,
    only_successful=True,
    verbose=False,
):
    """
    Iteratively collect and filter out specific runs.
    Returns filtered dates_dict
    """
    out_dates_dict = dict()
    ...
    for single_date in daterange(dt_start_date, dt_end_date):
        ...
        date_dict = dates_dict.get(current_date_str, {})
        # filter date_dict
        date_dict = filter_date_dict(
            date_dict, percentile_value, challenges, only_successful
        )
...
def iteratively_evaluate_dates_files_runwise(
    eval_run_function,
    start_date,
    end_date,
    load_only_challenge_data=True,
    challenges=True,
    only_successful=True,
    fast_runs_percentiles=None,
    run_arguments=None,
    ...
    """
    Iteratively evaluate date files in range start_date to end_date. On each run the eval_run_fuction is executed.
    Returns list of evaluation function returns (one for each run)
    """
    all_returns = []
        ...
        for date_key in dates_dict:
            ...
            if fast_runs_percentiles is not None:
                for id_fast_runs_percentile, fast_runs_percentile in enumerate(
                    ...
                    # filter good runs
                    fast_runs, fast_ids_runs = get_fast_runs(
                        runs, ids_runs, run_lengths, percentile=fast_runs_percentile
                    )
                ...
                for id_run, run in enumerate(runs):
                    ...
                    all_returns.append(
                        eval_run_function(
                            run, ids_runs[id_run], date_dict, run_arguments, plot=False
                        )
                    )
...
def iteratively_evaluate_all_runs_in_dates_dict(
    eval_run_function,
    dates_dict,
    start_date=None,
    end_date=None,
    challenges=True,
    only_successful=True,
    fast_runs_percentile=None,
    run_arguments=None,
    plot=False,
    verbose=False,
):
    """
    Iteratively evaluate date files in range start_date to end_date. On each date the eval_run_fuction is executed.
    Returns list of evaluation function returns (one for each run)
    """
    all_returns = []
...
def filter_date_dict(
    date_dict, percentile_value=200, challenges=True, only_successful=True
):
    """
    Filter runs in date dicts
    """
    ...
    if percentile_value is not None:
        ...
        run_lengths = np.asarray(run_lengths)[ids_runs]
        runs, ids_runs = get_fast_runs(
            runs, ids_runs, run_lengths, percentile=percentile_value
        )
