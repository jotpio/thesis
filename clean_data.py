...
def clean_data(dates_dict, debug=False):
    """
    Clean data by checking contraints of challenge runs:
    - always same number of fish
    - number of fish corresponds to difficulty
    - remove too short runs
    - target fish must be at set position
    """
    ...
    for date_key in dates_keys:
        ...
        assert (
            len(runs) == len(all_difficulties) == len(all_challenge_runs)
        ), f"{len(runs),len(all_difficulties),len(all_challenge_runs)}"
        ...
        for id_run, run in enumerate(runs):
            ...
            run, previous_cutoff = start_check_if_num_fish_constant_for_run(
                run_fish, run_difficulty, run, debug=debug
            )
            ...
            assert len(fish_pos_this_run) == len(
                run_fish
            ), f"{len(fish_pos_this_run)}\n{len(fish_pos_this_run)}"
            run, previous_cutoff = start_check_if_target_in_fixed_starting_pos(
                fish_pos_this_run, run, max_dist_to_starting_pos, debug=debug
            )
            run_fish = run_fish[previous_cutoff:]
            fish_pos_this_run = fish_pos_this_run[previous_cutoff:]

            # cut off end
            run, end_cutoff = end_check_if_num_fish_constant_for_run(
                run_fish, run_difficulty, run, debug=debug
            )
...
def start_check_if_num_fish_constant_for_run(
    run_fish, run_difficulty, run, debug=False
):
    """
    Check if number of fish constant at start of run
    """
    ...
    for id_ts, fish_in_ts in enumerate(run_fish):
        if run_difficulty != len(fish_in_ts) - 1:
            if id_ts == current_cut_off + 1:
                ...
                if debug:
                    print(
                        f"\tLater jump in number of fish found which is not in first consecutive time steps({current_cut_off} and {id_ts}) : {run}, number of fish:  {run_difficulty+1} expected, got  {len(fish_in_ts)}"
                    )
                    break
    ...
    if current_cut_off != -1:
        ...
        if (run[1] - run[0]) > current_cut_off:
            if debug:
                print(
                    f"\tCutting off {current_cut_off+1} from beginning of {run} because number of fish not consistent with difficulty"
                )
            run[0] = run[0] + current_cut_off + 1
        else:
            if debug:
                print(
                    f"\tRun too short to be cut off... Removing instead: (run-{run}; cut-off:{current_cut_off}; run length:{run[1] - run[0]}) - START const num fish"
                )
...
def start_check_if_target_in_fixed_starting_pos(
    fish_pos_this_run, run, max_dist_to_starting_pos, debug=False
):
    """
    Check if target fish at set position at start of run
    """
    ...
    if len(fish_pos_this_run) > 0:
        assert len(fish_pos_this_run) == (
            run[1] - run[0] + 1
        ), f"Wrong fish positions? len fish: {len(fish_pos_this_run)}; len run: {run[1] - run[0]+1}; run: {run}"
        ...
        if current_cut_off != -1:
            ...
            if (run[1] - run[0]) > current_cut_off:
                if debug:
                    print(
                        f"\tCutting off {current_cut_off+1} from beginning of {run} because fish is not in starting position"
                    )
                run[0] = run[0] + current_cut_off + 1
            else:
                if debug:
                    print(
                        f"\tRun to short to be cut off... Removing instead: (run-{run}; cut-off:{current_cut_off}; run length:{run[1] - run[0]}) - START target pos"
                    )
                    run[1] = run[0]
...
def end_check_if_num_fish_constant_for_run(run_fish, run_difficulty, run, debug=False):
    """
    Check if number of fish constant at end of run
    """
    # compare num of fish in ts to expected number of fish (difficulty+1)
    cut_off = -1
    ...
    if len(run_fish) > 0:
        assert len(run_fish) == (
            run[1] - run[0] + 1
        ), f"bad run_fish?: len run fish-{len(run_fish)}, len run-{run[1] - run[0]+1}"

        for id_ts, fish_in_ts in enumerate(run_fish):
            if run_difficulty != len(fish_in_ts) - 1:
                if debug:
                    print(
                        f"\tEND: {run_difficulty} and {len(fish_in_ts)-1} different in timestep {id_ts}"
                    )
                cut_off = id_ts
        ...
        if cut_off != -1:
            ...
            if (run[1] - run[0]) > cut_off:
                if debug:
                    print(
                        f"\tCutting off at index {cut_off+1} from end of {run} because number of fish not consistent with difficulty"
                    )
                run[1] = run[0] + cut_off - 1
            else:
                if debug:
                    print(
                        f"\tRun to short to be cut off... Removing instead: run-{run}, cut_off:{cut_off}, run length:{run[1] - run[0]} - END const num fish"
                    )
                    run[1] = run[0]
        # print(f"inside diff {run}")
    return run, cut_off
