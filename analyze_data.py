...
from util import (
    ...
    get_successful_runs,
    get_challenge_runs,
    get_fish_pos_per_run,
    get_fish_dir_per_run,
)
...
def get_magnitude_of_diff_dir_to_target_and_dir_robot(
    robot_positions, target_positions, robot_directions, rolling_mean=False
):
    """
    Get magnitude of difference of current robot vector towards target and current robot direction
    """
    # calcalate direction towards target
    directions_to_target = [
        normalize(tpos - rpos)
        for (rpos, tpos) in zip(robot_positions, target_positions)
    ]
    # get magnitude of diff of direction to target and direction of robot
    magnitude_of_diff_dir_to_target_and_dir_robot = [
        np.linalg.norm(dirtotar - rdir, axis=0)
        for (rdir, dirtotar) in zip(robot_directions, directions_to_target)
    ]
    ...
    if rolling_mean:
        ...
        magnitude_of_diff_dir_to_target_and_dir_robot = uniform_filter1d(
            magnitude_of_diff_dir_to_target_and_dir_robot, size=5
        )
...
def flatten_plateau_rolling_window(array, window_size, threshold):
    """
    Flatten parts of data where it does not change much using a rolling window
    """
    ...
    for id_point, point in enumerate(flattened_array):
        if id_point + window_size < len(flattened_array):
            ...
            if np.std(window_array) < threshold:
                flattened_array[id_point : id_point + window_size] = [
                    np.mean(window_array)
                ] * window_size
        else:
            last_window = flatten_plateau_rolling_window(
                flattened_array[id_point : len(array)], window_size - 1, threshold
            )
            try:
    ...
    for id_point, point in enumerate(flattened_array):
        if not current_plateau:
            ...
            if np.std(current_plateau) > threshold:
                ...
                flattened_array[current_plateau_ids[0] : current_plateau_ids[1] + 1] = [
                    np.mean(current_plateau)
                ] * len(current_plateau)
...
def get_run_turns_towards_target(
    fish1_pos_this_run,
    robot_pos_run,
    robot_dir_run,
    run_target_dists,
    prominence_target_dist=20,
    plot=False,
):
    """
    Calculate number of turns towards the target in a given run
    """
    turning_points = []

    assert len(fish1_pos_this_run) == len(robot_pos_run), print(
        f"fish:{len(fish1_pos_this_run)}; robot:{len(robot_pos_run)}"
    )

    # flatten plateaus before finding peaks in metric robot direction - dir towards target
    metric_dir_robot_dir_tow_target = get_magnitude_of_diff_dir_to_target_and_dir_robot(
        robot_pos_run, fish1_pos_this_run, robot_dir_run, rolling_mean=True
    )
    flat_metric_dir_robot_dir_tow_target = np.array(
        flatten_plateaus(metric_dir_robot_dir_tow_target, threshold=0.1)
    )
    # get valley points towards target
    metric_peaks, metric_peaks_properties = signal.find_peaks(
        np.asarray(flat_metric_dir_robot_dir_tow_target),
        distance=3,
        prominence=0.1,
        plateau_size=1,
    )
    ...
    target_dist_peaks, peaks_properties = signal.find_peaks(
        np.asarray(flat_run_target_dists), distance=3, prominence=prominence_target_dist
    )
    ...
    for id_peak, dist_peak in enumerate(target_dist_peaks):
        for peak_prop_zip in zip(metric_peaks, metric_peaks_properties["right_edges"]):
            if (
                np.abs(dist_peak - peak_prop_zip[0]) < 20
                or np.abs(dist_peak - peak_prop_zip[1]) < 20
            ):
    ...
    if plot:
        ...
        ax1.scatter(
            metric_peaks,
            np.asarray(flat_metric_dir_robot_dir_tow_target)[metric_peaks],
            color="b",
        )
        ...
        ax2.scatter(
            target_dist_peaks, flat_run_target_dists[target_dist_peaks], color="m"
        )
        ax2.scatter(
            turning_points, flat_run_target_dists[turning_points], color="r", s=100
        )

        ax1.set_xlabel("X timesteps (frames)")
        ax1.set_ylabel(
            "Y1 magnitude of diff of dir robot and dir towards target", color="g"
        )
...
def calculate_tortuosity(dates_dict, only_successful=True, plot=False):
    """
    Calculate both straightness index and path sinuosity for all runs in dates in dates dict
    """
    ...
    for date_key in dates_dict:
        ...
        for id_run, run in enumerate(runs):
            ...
            fish1_dists_per_step = np.abs(
                np.linalg.norm(np.diff(fish1_pos_this_run), axis=1)
            )
            robot_dists_per_step = np.abs(
                np.linalg.norm(np.diff(robot_pos_run), axis=1)
            )
            ...
            fish1_dist_start_end = np.abs(
                np.linalg.norm(fish1_pos_this_run[0] - fish1_pos_this_run[-1])
            )
            robot_dist_start_end = np.abs(
                np.linalg.norm(robot_pos_run[0] - robot_pos_run[-1])
            )
            # get tortuosity (track length / distance between start and end points)
            fish1_tortuosity_straightness_index = (
                fish1_dist_start_end / fish1_dist_traveled
                if fish1_dist_traveled != 0
                else 0
            )
            robot_tortuosity_straightness_index = (
                robot_dist_start_end / robot_dist_traveled
                if robot_dist_traveled != 0
                else 0
            )
            #
            date_fish1_tortuosity_straightness_index.append(
                fish1_tortuosity_straightness_index
            )
            date_robot_tortuosity_straightness_index.append(
                robot_tortuosity_straightness_index
            )
            ...
            """
            robot_calc_turn_angle = lambda x: np.arctan2(x[1][1], x[1][0]) - np.arctan2(
                x[0][1], x[0][0]
            )  # difference between vec1 angle to x axis and vec2 angle to x axis

            # robot
            robot_turning_angles = forward_rolling_window_apply(
                robot_dir_run, window_size=2, function=robot_calc_turn_angle
            )
            for id_angle, angle in enumerate(
                robot_turning_angles
            ):  # normalize angles in to [-PI,PI]
                if angle > np.pi:
                    angle -= 2 * np.pi
            ...
            robot_mean_cosine_of_turning_angles = np.mean(
                robot_cosines_of_turning_angles
            )
            robot_mean_step_length = np.mean(robot_dists_per_step)
            robot_coefficient_of_variation_of_step_length = (
                np.std(robot_dists_per_step) / robot_mean_step_length
            )

            c = robot_mean_cosine_of_turning_angles
            b = robot_coefficient_of_variation_of_step_length
            p = robot_mean_step_length
            robot_sinuosity = 2 * np.power(
                (p * (((1 + c) / (1 - c)) + np.power(b, 2))), -0.5
            )
            ...
            fish1_turning_angles = forward_rolling_window_apply(
                fish1_dir_run, window_size=2, function=fish1_calc_turning_angle
            )
            for id_angle, angle in enumerate(
                fish1_turning_angles
            ):  # normalize angles in to [-PI,PI]
                if angle > np.pi:
                    angle -= 2 * np.pi
            ...
            fish1_mean_cosine_of_turning_angles = np.mean(
                fish1_cosines_of_turning_angles
            )
            fish1_mean_step_length = np.mean(fish1_dists_per_step)
            fish1_coefficient_of_variation_of_step_length = (
                np.std(fish1_dists_per_step) / fish1_mean_step_length
            )

            c = fish1_mean_cosine_of_turning_angles
            b = fish1_coefficient_of_variation_of_step_length
            p = fish1_mean_step_length
            fish1_sinuosity = 2 * np.power(
                (p * (((1 + c) / (1 - c)) + np.power(b, 2))), -0.5
            )
        ...
        all_dates_fish1_tortuosity_straightness_index.extend(
            date_fish1_tortuosity_straightness_index
        )
        all_dates_robot_tortuosity_straightness_index.extend(
            date_robot_tortuosity_straightness_index
        )
    ...
    if plot:
        ...
        plt.title(
            "straightness index of robot and target fish path (distance_start_end_point / distance_traveled)"
        )
        plt.hist(
            all_dates_fish1_tortuosity_straightness_index,
            bins=bins,
            color="b",
            alpha=0.5,
            label="fish1 straightness index",
        )
        plt.hist(
            all_dates_robot_tortuosity_straightness_index,
            bins=bins,
            color="g",
            alpha=0.5,
            label="robot straightness index",
        )
...
def get_run_turns_curvature(timestamps_run, robot_pos_run, plot=False):
    ...
    dt_timestamps = [
        ...
        for timestamp in timestamps_run
    ]
    ...
    speed_run = np.linalg.norm(
        velocity_vectors_run, axis=1
    ...
    K1 = np.abs(
        velocity_x_run[0:-1] * acceleration_y_run
        - velocity_y_run[0:-1] * acceleration_x_run
    )
    K2 = np.power(
        (np.power(velocity_x_run[0:-1], 2) + np.power(velocity_y_run[0:-1], 2)), (3 / 2)
    )
    K = np.empty_like(K1)
    K[:] = np.nan
    ...
    if not np.isnan(K).all():
        ...
        peaks, peak_properties = signal.find_peaks(
            np.asarray(K), height=threshold, prominence=0.01
        )
    else:
    ...
    if plot:
        ...
        plt.scatter(
            robot_pos_run[:, 0][peaks],
            robot_pos_run[:, 1][peaks],
            marker="o",
            color="r",
        )
        plt.show()
