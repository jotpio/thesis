...
from util import (
    ...
    calculate_run_velocity_speed_acceleration,
    get_fish_following_per_run,
    flatten_2d_list,
    tolerant_mean,
    equalize_arrays,
)
...
def plot_all_positions(
    ...
    for date_key in dates_keys:
        ...
        if date_positions is not None:
            if only_successful:
                successful_runs, _ = get_successful_runs(
                    date_dict["runs"], date_dict["successful"]
                )
                ...
                challenge_runs, _ = get_challenge_runs(
                    date_dict["runs"], date_dict["challenges"]
                )
    ...
    if len(all_positions) != 0:
        ...
        ax.set_title(
            f"all positions: {start_date.strftime('%Y-%m-%d')}  -  {end_date.strftime('%Y-%m-%d')} - {title_run_modifier}"
        )
...
def plot_run(
    date_dict, id_run, date_key, challenges=True, only_successful=True, show=True
):
    ...
    dt_timestamps = [
        ...
        for timestamp in date_dict["timestamps"]
    ]
    (
        velocity_vectors_run,
        speed_run,
        acceleration_run,
    ) = calculate_run_velocity_speed_acceleration(date_dict, run, dt_timestamps)
    ...
    fig.suptitle(
        f"{dt_timestamps[run[0]]} - {dt_timestamps[run[1]]}: {title_run_modifier}"
    )  # , fontsize=10)
    plt.tight_layout()
    if show:
        plt.show()
...
def plot_runs(
    ...
    for date_key in dates_keys:
        ...
        if len(date_dict["positions"]) != 0:
            ...
            if only_successful:
                runs, ids_runs = get_successful_runs(
                    date_dict["runs"], date_dict["successful"]
                )

            if challenges:
                runs, ids_runs = get_challenge_runs(
                    date_dict["runs"], date_dict["challenges"]
                )
            else:
                runs = date_dict["runs"]
...
def plot_position_hexmap(pos, ax=None):
    ...
    ax.invert_yaxis()
    ax.hexbin(
        x, y, gridsize=80, bins="log", cmap="inferno"
    )  # use log bins to ignore positions where robot stands still
    plt.show()


def plot_starts_ends(
    dates_dict,
    start_date=None,
    end_date=None,
    challenges=True,
    ...
    if len(all_starts) > 0:
        ...
        plt.scatter(
            np.array(all_starts)[:, 0],
            np.array(all_starts)[:, 1],
            marker="o",
            s=15,
            color="green",
        ...
        if only_successful:
            ...
            run_descriptor = ""
        plt.title(
            f"[{start_date}]-[{end_date}] - all robot start and end positions of {run_descriptor}runs"
        )
        plt.xlim(0, 2000)
        plt.ylim(0, 2000)
...
def plot_rotations_and_heatmap(
    dates_dict,
    start_date=None,
    end_date=None,
    challenges=True,
    ...
    for date_key in dates_keys:
        ...
        if only_successful:
            successful_runs, ids = get_successful_runs(
                date_dict["runs"], date_dict["successful"]
            )
            ...
            challenge_runs, ids = get_challenge_runs(
                date_dict["runs"], date_dict["challenges"]
            )
    ...
    fig.suptitle(
        f"[{start_date}]-[{end_date}] - robot: Average rotation circular histogram and positional heatmap",
        fontsize=14,
    )
...
def plot_inter_individual_distances(
    dates_dict_robot_fish,
    start_date=None,
    end_date=None,
    challenges=True,
    only_successful=True,
    ...
    for date_key in dates_keys:
        ...
        for id_run, run in enumerate(runs):
            ...
            if len(fish_pos_this_run) != len(robot_pos_run):
                assert (
                    False
                ), f"Wrong array lengths: fish {len(fish_pos_this_run)} and robot {len(robot_pos_run)}"
            fish1_pos_this_run = np.array(
                [fish[0] for fish in fish_pos_this_run]
            )  # get first fish for each timestamp as it always is the target fish
            # plot all runs in one
            ii_distances_r_f1_run = np.linalg.norm(
                fish1_pos_this_run - robot_pos_run, axis=1
            )
            plt.figure(
                num=f"{date_key} - inter-individual distances between robot and target fish (id 1)",
                figsize=(15, 5),
            )
            ...
            hist, bin_edges = np.histogram(
                indexes, bins=bins, range=None, weights=None, density=None
            )  # bin the indexes to get sclice edges
        ...
        figs1.append(fig)
        plt.title(
            f"{date_key} - inter-individual distances between robot and target fish (id 1)"
        )
        plt.ylim(0, 3000)
        ...
        fig = plt.figure(
            num=f"{date_key} - frame-scliced inter-individual distances between robot and target fish (id 1)",
            figsize=(15, 5),
        )
        figs2.append(fig)
        ...
        plt.bar(
            x=ticks,
            height=overall_mean_iid_per_bin,
            yerr=overall_std_iid_per_bin,
            tick_label=ticks,
            color="black",
            ecolor="gray",
            capsize=4,
        ...
        plt.title(
            f"{date_key} - frame-binned average inter-individual distances between robot and target fish (id 1)"
        )
        ...
        """
        plt.axhline(
            40, linewidth=2, color="r", linestyle=(0, (2, 2)), label="repulsion"
        )  # repulsion distance
        plt.axhline(
            100, linewidth=2, color="b", linestyle=(0, (2, 2)), label="orientation"
        )  # orientation distance
        plt.axhline(
            220, linewidth=2, color="g", linestyle=(0, (2, 2)), label="attraction"
        )  # attraction distance
...
def plot_run_length_hist(
    ...
    for date_key in dates_keys:
        ...
        if only_successful:
            run_lengths, _ = get_successful_runs(
                date_run_lengths, date_dict["successful"]
            )
        elif challenges:
            run_lengths, _ = get_challenge_runs(
                date_run_lengths, date_dict["challenges"]
            )
        else:
...
def plot_robot_distance_goal(
    dates_dict, start_date, end_date, challenges=True, only_successful=True, show=True
):
    dates_keys = dates_dict.keys()
...
def plot_following1(
    ...
    for date_key in dates_keys:
        ...
        for id_run, run in enumerate(runs):
            ...
            percentage_of_run_following = target_fish_following_this_run.sum() / len(
                target_fish_following_this_run
            )
    ...
    flattened_percentage_of_run_following = [
        x for xs in daily_percentage_of_run_following for x in xs
    ]
    fig = plt.figure(figsize=(15, 8))
    plt.title(
        f"[{start_date}] - [{end_date}] - histogram of leading percentages per run "
    )
    ...
    fig = plt.figure(figsize=(15, 8))
    plt.title(
        f"[{start_date}] - [{end_date}] - correlation plot of run lengths and leading percentage"
    )
    ...
    # linear regression
    coef = np.polyfit(
        flattened_daily_run_lengths, flattened_percentage_of_run_following, 1
    )
    poly1d_fn = np.poly1d(coef)
    # poly1d_fn is now a function which takes in x and returns an estimate for y
    plt.plot(
        flattened_daily_run_lengths,
        flattened_percentage_of_run_following,
        "bo",
        flattened_daily_run_lengths,
        poly1d_fn(flattened_daily_run_lengths),
        "--k",
    ...
    mymodel = np.poly1d(
        np.polyfit(
            flattened_daily_run_lengths, flattened_percentage_of_run_following, 10
        )
    )
...
def plot_runlength_dist_goal_target_corr(
    dates_dict,
    start_date=None,
    end_date=None,
    only_successful=True,
    ...
    for date_key in dates_keys:
        ...
        for id_run, run in enumerate(runs):
            ...
            if len(all_fish_pos_this_run) != len(robot_pos_run):
                print(
                    f"Wrong array lengths: fish and robot {run} {len(all_fish_pos_this_run)} {len(robot_pos_run)}"
                )
                assert False
            # else:
            #     print("all good")
    ...
    flat_daily_initial_dist_robot_target_fish = flatten_2d_list(
        daily_initial_dist_robot_target_fish
    )
    ...
    plt.scatter(
        flat_daily_run_lengths,
        flat_daily_initial_dist_robot_target_fish,
        label="distance to target fish",
        s=3,
    )
    plt.scatter(
        flat_daily_run_lengths,
        flat_daily_initial_dist_robot_goal,
    ...
    plt.title(
        f"[{start_date}] - [{end_date}] - correlation from run length to distance to goal and target fish"
    )
    plt.legend()
    if show:
        plt.show()
    ...
    plt.scatter(
        flat_daily_run_lengths,
        np.abs(
            np.array(flat_daily_initial_dist_robot_goal)
            - np.array(flat_daily_initial_dist_robot_target_fish)
        ),
        s=3,
    )
    plt.title(
        f"[{start_date}] - [{end_date}] - correlation from run length to difference of distance to goal and distance to target"
    )
    plt.xlabel("run length in seconds")
