...
def extend_robot_data(dates_dict, verbose=False):
    """
    Extend data by adding successful, day_length, and run lengths
    """
    # parameters
        ...
        if len(timestamps) > 1:
            ...
            for id_run, run in enumerate(runs):
                ...
                start_time = datetime.strptime(
                    timestamps[run[0]], "%Y-%m-%d %H:%M:%S,%f"
                )
            ...
            if len(date_run_lengths) > 0:
                ...
                if verbose:
                    print(
                        f"{date_key}: Mean run length: {mean_length:.2f}; median run length: {median_length:.2f}"
                    )
    # add successful runs
...
def get_successful_runs(dates_dict, verbose=False):
    """
    Check for each run in successful by checking following status and robot position in target
    """
    ### get successful runs
    for date_key in dates_dict.keys():
            ...
            for i in range(run[1] - run[0] - 1):
                ...
                if run_end_pos[0] < 750 and run_end_pos[1] > 1250:
                    ...
                    if len(date_dict_fish) > run[1] - i:
                        ...
                        if verbose:
                            print(
                                "Number of fish data entries is lower that number of timestamps"
                            )
                        break
                    for fish in ts_fish:
                        if fish["id"] == 1:
        ...
        if len(successful_ch) > 0 and len(ch_runs) > 0:
            ...
            if verbose:
                print(
                    f"{date_key}: Number of challenge runs:{len(ch_runs)}, successful:{len(successful_challenge_runs)} | {len(successful_challenge_runs) /len(ch_runs) * 100:.2f}% successful runs"
                )
        else:
            if verbose:
                print(
                    f"number of challenge runs: {len(ch_runs)}; number of successful runs: {len(successful_ch)}"
                )
