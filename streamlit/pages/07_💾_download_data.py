...
class DownloadPage(Page):
    ...
    def run_page(self, title, description):
        ...
        #PAGE
        #
        st.write(f"Click the button to download data files for the date range: {self.data_model.start_date} - {self.data_model.end_date}")

        prepare = st.button("Prepare data for download")
        with st.spinner("Preparing ZIP file..."):
            if prepare:
                zip_name = zip_data(self.data_model.start_date, self.data_model.end_date, self.data_model.dates_dict, self.data_model.only_challenges_loaded)
